...
export default function AdminFAQs() {
  ...
  const handleSave = () => {
    ...
    if (editing) {
      updateFaq(editing.id, form);
      toast.success('FAQ updated!');
    } else {
      addFaq({ ...form, id: `f-${Date.now()}` });
        ...
        {faqs.map(faq => (
          <div key={faq.id} className="bg-white rounded-xl shadow-sm p-5 flex items-start gap-4">
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <button onClick={() => { if(confirm('Delete?')) { deleteFaq(faq.id); toast.success('Deleted'); }}} className="p-2 hover:bg-stone-100 rounded-lg text-stone-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
